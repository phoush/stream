...
def get_outer_tmap_loop_dimensions(workload, accelerator, node) -> List[TemporalLoop]:
    ...
    """
    # TODO DILATION NOT SUPPORTED!!!

    outer_loops = []
    core = next((x for x in accelerator.cores.nodes() if x.id == node.core_allocation),None)
    sm_current_node = node.mappings[core].spatial_mapping_int.mapping_dict_origin
    sm_current_node = [j for i in sm_current_node['O'] for j in i]
    tm_current_node = node.mappings[core].temporal_mapping.mapping_dic_origin
    tm_current_node = [j for i in tm_current_node['O'] for j in i]

    # find outer producer loops based only on TM
    producer_outer_loops_tm = []
    for ii_tm, tm_loop in enumerate(tm_current_node):
        if all([x[0] not in ['C','FX','FY'] for x in tm_current_node[ii_tm:]]):
            producer_outer_loops_tm = tm_current_node[ii_tm:]
    # find outer producer loops based on successor SM
    sm_successors = []
    producer_outer_loops_sm = []
    tm_extra_list = []
    successor_nodes = []
    queue = [x for x in workload.successors(node)]
    from copy import deepcopy

    while queue != []:
        tmp_queue = queue.copy()
        for successor in tmp_queue:
            if not isinstance(successor, ComputationNode):
                queue.remove(successor)
                queue += [x for x in workload.successors(successor)]
            else:
                successor_nodes.append(successor)
                queue.remove(successor)
    for successor in successor_nodes:
        producer_outer_loops_sm.append([])
        successor_core = next((x for x in accelerator.cores.nodes() if x.id == successor.core_allocation),None)
        sm_successor = successor.mappings[successor_core].spatial_mapping_int.mapping_dict_origin
        sm_successor = [j for i in sm_successor['O'] for j in i if j[0] != 'K']
        loop_req = {'K':1,'OX':1,'OY':1}
        sm_successor_loops = {'C':1,'OX':1,'OY':1,'FX':1,'FY':1}
        for sm in sm_successor:
            sm_successor_loops[sm[0]] *= sm[1]
        loop_req['K'] = sm_successor_loops['C']
        ...
        loop_index = 0
        while not all([x <= 1 for x in loop_req.values()]):
            try:
                if tm_current_node[loop_index][0] in ['K','OX','OY']:
                    loop_req[tm_current_node[loop_index][0]] /= tm_current_node[loop_index][1]
                loop_index += 1
            except:
                breakpoint()
        loop_index -= 1
        for tm in tm_current_node[loop_index+1:]:
            producer_outer_loops_sm[-1].append(tm)

        if loop_index >= 0:
            if loop_req[tm_current_node[loop_index][0]] < 1:
                tm_extra = (tm_current_node[loop_index][0], int(tm_current_node[loop_index][1]/loop_req[tm_current_node[loop_index][0]]))
                tm_extra_inner = (tm_current_node[loop_index][0], int(loop_req[tm_current_node[loop_index][0]]))
                producer_outer_loops_sm[-1].insert(0, tm_extra)
                tm_extra_list.append(tm_extra_inner)
            else:
                tm_extra_list.append([])
        else:
            tm_extra_list.append([])

    producer_outer_loops = producer_outer_loops_sm + [producer_outer_loops_tm]
    tm_extra_list.append([])
    producer_outer_loops_len = [np.prod([x[1] for x in y]) for y in producer_outer_loops]
    producer_outer_loops = producer_outer_loops[producer_outer_loops_len.index(min(producer_outer_loops_len))]
    tm_extra_inner_loop = tm_extra_list[producer_outer_loops_len.index(min(producer_outer_loops_len))]
    producer_inner_loops = []
    for tm in tm_current_node[:len(tm_current_node)-len(producer_outer_loops)]:
        producer_inner_loops.append(tm)
    if tm_extra_inner_loop != []:
        producer_inner_loops.append(tm_extra_inner_loop)

    #print('TM',tm_current_node)
    #print('SM',sm_current_node)
    #print('TM outer loop', producer_outer_loops)
    #print('TM inner loop', producer_inner_loops)
    node.producer_inner_loops = producer_inner_loops
#    # find outer loops based on predecessor node
#    consumer_outer_loops = []
#    for predecessor in workload.predecessors(node):
#        consumer_outer_loops.append([])
#        predecessor_core = next((x for x in accelerator.cores.nodes() if x.id == predecessor.core_allocation),None)
#        sm_predecessor = predecessor.mappings[predecessor_core].spatial_mapping_int.mapping_dict_origin
#        sm_predecessor = [j for i in sm_predecessor['O'] for j in i if j[0] not in ['C','FX','FY']]
#        sm_predecessor += [x for x in predecessor.producer_inner_loops if x[0] not in ['C','FX','FY']]
#        loop_req = {'C':1,'OX':1,'OY':1}
#        sm_predecessor_loops_available = {'K':1,'OX':1,'OY':1}
#        for sm in sm_predecessor:
#            sm_predecessor_loops_available[sm[0]] *= sm[1]
#        
#        sm_predecessor_loops_available['K'] /= np.prod([x[1] for x in sm_current_node if x[0] == 'C'])
#        
#        if 'OX' in [x[0] for x in sm_predecessor]:
#            sm_predecessor_loops_available['OX'] /= (np.prod([x[1] for x in sm_current_node if x[0] == 'OX'])*node.stride['IX'])
#            sm_predecessor_loops_available['OY'] /= (np.prod([x[1] for x in sm_current_node if x[0] == 'OY'])*node.stride['IY'])
#
#        loop_index = 0
#        while not all([x <= 1 for x in sm_predecessor_loops_available.values()]):
#            if tm_current_node[loop_index][0] in ['C','OX','OY']:
#                if tm_current_node[loop_index][0] == 'OX':
#                    sm_predecessor_loops_available[tm_current_node[loop_index][0]] /= (tm_current_node[loop_index][1]*node.stride['IX'])
#                elif tm_current_node[loop_index][0] == 'OY':
#                    sm_predecessor_loops_available[tm_current_node[loop_index][0]] /= (tm_current_node[loop_index][1]*node.stride['IY'])
#                else:
#                    sm_predecessor_loops_available['K'] /= tm_current_node[loop_index][1]
#            loop_index += 1
#        breakpoint()
#        for tm in tm_current_node[loop_index+1:]:
#            consumer_outer_loops[-1].append(tm)
#        if tm_current_node[loop_index][0] == 'C':
#            if sm_predecessor_loops_available['K'] < 1:
#                tm_extra = ('C', int(tm_current_node[loop_index][1]/sm_predecessor_loops_available['C']))
#                consumer_outer_loops[-1].insert(0, tm_extra)
#        else:
#            if sm_predecessor_loops_available[tm_current_node[loop_index][0]] < 1:
#                tm_extra = (tm_current_node[loop_index][0], int(tm_current_node[loop_index][1]/ sm_predecessor_loops_available[tm_current_node[loop_index][0]]))
#                consumer_outer_loops[-1].insert(0, tm_extra)
#
#
#    print("prod:",producer_outer_loops)
#    print("cons:",consumer_outer_loops)
#    outer_loops = []
#    if  np.prod([x[1] for x in producer_outer_loops]) > np.prod([x[1] for x in consumer_outer_loops]):
#        for tm in consumer_outer_loops:
#            outer_loops.append(TemporalLoop(tm[0],tm[1]))
#    else:
#        for tm in producer_outer_loops:
#            outer_loops.append(TemporalLoop(tm[0],tm[1]))
    for tm in producer_outer_loops:
        outer_loops.append(TemporalLoop(tm[0],tm[1]))
    ...
    if not outer_loops:
        outer_loops.append(TemporalLoop('B', 1))
    return outer_loops

def get_non_type_predecessors(node, types, workload):
    """Find all workload nodes that are not of any type in types.
    ...
    """
    preds = list(workload.predecessors(node))
    while any([type(pred) in types for pred in preds]):
        ...
        skip_node_preds = list(workload.predecessors(skip_node))
...
def get_finer_nodes(
    ...
    for n in range(nb_cns):
        ...
        finer_node = ComputationNode(
            ...
            group_id=group_id,
            mappings=original_node.mappings.copy()
        )
        ...
        finer_node.producer_inner_loops = original_node.producer_inner_loops.copy()
...
def get_inter_edges_rtree(
    producer, consumer, finer_producers, finer_consumers, workload
):
    ...
    for operand, parent_node in consumer.input_operand_source.items():
        if parent_node == producer:
            ...
            non_dummy_parents = get_non_type_predecessors(
                parent_node, [DummyNode], workload
            )
...
def get_tensor_cns(node, finer_nodes, workload):
    is_source_node = len(get_non_type_predecessors(node, [DummyNode], workload)) == 0
...
def propagate_cn_production_for_non_cn(node, input_tensor):
...
def get_inter_edges_numpy(
    ...
    paths_between_generator = nx.all_simple_paths(
        workload, source=producer, target=consumer
    )
    all_inter_edges = []
    for path_between in paths_between_generator:
        ...
        if node in numpy_tensors:
            ...
            finer_nodes = finer_nodes_dict[node]
            tensor_cns = get_tensor_cns(node, finer_nodes, workload)
        ...
        for i, node in enumerate(path_between[1:-1], start=1):
            ...
            tensor = propagate_cn_production_for_non_cn(node, tensor)
        ## LAST NODE IN PATH
        ...
        if last_node in numpy_tensors:
            ...
            finer_nodes = finer_nodes_dict[last_node]
            tensor_cns = get_tensor_cns(last_node, finer_nodes, workload)
            numpy_tensors[node] = tensor_cns
        last_tensor = tensor_cns[dependent_operand]
        inter_edges = get_inter_edges_tensor_based(tensor, last_tensor)
        for prod, cons in inter_edges:
...
def set_nb_real_predecessors(G):
...
def get_cn_graph(workload, accelerator):
    unique_finer_nodes = []
    ...
    for node in nx.topological_sort(workload):
        if not isinstance(
            ...
            continue
        outer_temporal_loops = get_outer_tmap_loop_dimensions(workload, accelerator, node)
        finer_nodes, unique_nodes = get_finer_nodes(node, outer_temporal_loops)
        #logger.info(f"{node}: Outer loops {outer_temporal_loops}.")
        #logger.info(f"{node}: Generated {len(finer_nodes)} finer nodes.")
        finer_nodes_dict[node] = finer_nodes
        unique_finer_nodes += unique_nodes
        # Compute the edges between nodes originating from one bigger node (intra-edges)
        intra_edges = get_intra_edges(finer_nodes)
    ...
    all_pairs = get_all_node_pairs(workload)
    for producer, consumer, is_complex in all_pairs:
        finer_producers = finer_nodes_dict[producer]
        finer_consumers = finer_nodes_dict[consumer]
    ...
    set_base_priority_of_nodes(G,finer_nodes_dict)
    # Set nb of real predecessors of all nodes in G
    set_nb_real_predecessors(G)
    return G

#    logger.info(f"Finer graph: {G}.")
#
#    kwargs = self.kwargs.copy()
#    kwargs["original_workload"] = pickle_deepcopy(workload)
#    kwargs["workload"] = G
